test_that("dm_flatten block constructor", {
 block <- new_dm_flatten_block()
 expect_s3_class(
   block,
   c("dm_flatten_block", "transform_block", "block")
 )

 # Test with initial values
 block2 <- new_dm_flatten_block(start_table = "adae", recursive = TRUE)
 expect_s3_class(block2, "dm_flatten_block")
})

test_that("dm_flatten block flattens dm to single table", {
 block <- new_dm_flatten_block(start_table = "events", recursive = TRUE)

 subjects <- data.frame(id = 1:3, name = c("a", "b", "c"))
 events <- data.frame(subject_id = c(1, 1, 2), event_type = c("x", "y", "z"))
 test_dm <- dm::dm(subjects = subjects, events = events) |>
   dm::dm_add_pk(subjects, id) |>
   dm::dm_add_fk(events, subject_id, subjects)

 testServer(
   blockr.core:::get_s3_method("block_server", block),
   {
     session$flushReact()
     result <- session$returned$result()

     # Should return a data frame, not a dm
     expect_s3_class(result, "data.frame")
     expect_false(inherits(result, "dm"))

     # Should have columns from both tables
     expect_true("event_type" %in% names(result))
     expect_true("name" %in% names(result))

     # Should have 3 rows (from events table)
     expect_equal(nrow(result), 3)
   },
   args = list(
     x = block,
     data = list(data = function() test_dm)
   )
 )
})

test_that("dm_flatten block with recursive = FALSE", {
 block <- new_dm_flatten_block(start_table = "events", recursive = FALSE)

 subjects <- data.frame(id = 1:3, name = c("a", "b", "c"))
 events <- data.frame(subject_id = c(1, 1, 2), event_type = c("x", "y", "z"))
 test_dm <- dm::dm(subjects = subjects, events = events) |>
   dm::dm_add_pk(subjects, id) |>
   dm::dm_add_fk(events, subject_id, subjects)

 testServer(
   blockr.core:::get_s3_method("block_server", block),
   {
     session$flushReact()
     result <- session$returned$result()

     expect_s3_class(result, "data.frame")
     # Non-recursive should still work for directly related tables
     expect_true("name" %in% names(result))
   },
   args = list(
     x = block,
     data = list(data = function() test_dm)
   )
 )
})

test_that("dm_flatten block state includes parameters", {
 block <- new_dm_flatten_block(start_table = "adae", recursive = TRUE)

 subjects <- data.frame(USUBJID = paste0("SUBJ-", 1:3))
 events <- data.frame(USUBJID = c("SUBJ-1", "SUBJ-2"), AETERM = c("x", "y"))
 test_dm <- dm::dm(adsl = subjects, adae = events)

 testServer(
   blockr.core:::get_s3_method("block_server", block),
   {
     session$flushReact()

     state <- session$returned$state
     expect_true("start_table" %in% names(state))
     expect_true("recursive" %in% names(state))
     expect_equal(state$start_table(), "adae")
     expect_equal(state$recursive(), TRUE)
   },
   args = list(
     x = block,
     data = list(data = function() test_dm)
   )
 )
})

test_that("dm_flatten block flattens filtered dm correctly", {
 # Create filtered dm
 adsl <- data.frame(
   USUBJID = paste0("SUBJ-", 1:5),
   AGE = c(45, 52, 38, 61, 29),
   SEX = c("M", "F", "M", "F", "M")
 )
 adlb <- data.frame(
   USUBJID = rep(paste0("SUBJ-", 1:5), each = 2),
   PARAMCD = rep(c("NEUT", "WBC"), 5),
   AVAL = c(4.5, 8.2, 6.1, 7.5, 3.2, 6.8, 5.5, 9.1, 4.8, 7.2)
 )
 adae <- data.frame(
   USUBJID = c("SUBJ-1", "SUBJ-1", "SUBJ-2", "SUBJ-4", "SUBJ-4", "SUBJ-4"),
   AETERM = c("Headache", "Nausea", "Fatigue", "Dizziness", "Headache", "Rash"),
   AESEV = c("MILD", "MODERATE", "MILD", "SEVERE", "MILD", "MODERATE")
 )

 test_dm <- dm::dm(adsl = adsl, adlb = adlb, adae = adae) |>
   dm::dm_add_pk(adsl, USUBJID) |>
   dm::dm_add_fk(adlb, USUBJID, adsl) |>
   dm::dm_add_fk(adae, USUBJID, adsl)

 # Filter to subjects with high neutrophils
 filtered_dm <- test_dm |>
   dm::dm_filter(adlb = (PARAMCD == "NEUT" & AVAL > 5))

 block <- new_dm_flatten_block(start_table = "adae", recursive = TRUE)

 testServer(
   blockr.core:::get_s3_method("block_server", block),
   {
     session$flushReact()
     result <- session$returned$result()

     expect_s3_class(result, "data.frame")

     # Should have columns from adae and adsl
     expect_true("AETERM" %in% names(result))
     expect_true("AESEV" %in% names(result))
     expect_true("AGE" %in% names(result))
     expect_true("SEX" %in% names(result))

     # Should only have filtered subjects
     expect_true(all(result$USUBJID %in% c("SUBJ-2", "SUBJ-4")))
     expect_equal(nrow(result), 4)
   },
   args = list(
     x = block,
     data = list(data = function() filtered_dm)
   )
 )
})
