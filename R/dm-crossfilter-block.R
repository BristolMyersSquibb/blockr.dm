...
dm_crossfilter_server_factory <- function(
  active_dims, filters, range_filters,
  measure, agg_func = NULL
) {
  function(id, data) {
    shiny::moduleServer(
      id,
      function(input, output, session) {
        ...
        # Invalidates on data/dims/measure, NOT on filters.
        ...
        crossfilter_data_for_dim <- function(tbl_name, exclude_dim) {
          ...
          if (!is.null(lookup_info)) {
            result <- tryCatch(
              lookup_crossfilter_data(
                lookup_info, tbl_name, exclude_dim,
        ...
        crossfilter_agg_for_dim <- function(tbl_name, dim) {
          ...
          if (!is.null(lookup_info)) {
            result <- tryCatch(
              lookup_crossfilter_agg(
                lookup_info, tbl_name, dim,
                r_filters(), r_range_filters()
              ),
              error = function(e) NULL
            )
        ...
        cf_agg_for_dim_measure <- function(
          tbl_name, dim, measure_table,
          measure_col
        ) {
          ...
          if (measure_table == tbl_name) {
            ...
            # Get measure table data, apply own filters,
        ...
        filtered_row_count <- shiny::reactive({
          ...
          if (!is.null(lookup_info)) {
            result <- tryCatch(
              lookup_crossfilter_counts(
                lookup_info, info$tables,
                info$table_names,
                r_filters(), r_range_filters()
              ),
              error = function(e) NULL
          ...
          for (tbl_name in info$table_names) {
            ...
            filtered <- filtered + nrow(
              apply_crossfilter_filters(df, cat_f, rng_f)
            )
          }
        ...
        shiny::observeEvent(input$table_click, {
          ...
          if (!is.null(value)) {
            ...
            if (value %in% current_vals) {
              ...
              tbl_filters[[dim]] <- if (length(current_vals) == 0) {
                NULL
              } else {
                current_vals
              }
        ...
        build_filter_table <- function(tbl_name, dim) {
          ...
          columns_list[[value_col]] <- reactable::colDef(
            ...
            cell = function(value, index) {
              ...
              if (has_negative) {
                shiny::div(
                  style = "display: flex; align-items: center; justify-content: flex-end; gap: 6px;", # nolint
                  shiny::div(
                    style = "flex: 1; max-width: 100px; height: 14px; display: flex; position: relative;", # nolint
                    shiny::div(
                      style = "width: 50%; height: 100%; background: #f0f0f0; border-radius: 2px 0 0 2px; display: flex; justify-content: flex-end; overflow: hidden;", # nolint
                      if (is_negative) {
                        shiny::div(
                          style = sprintf(
                    ...
                    shiny::div(style = "width: 1px; height: 100%; background: #ccc;"), # nolint
                    shiny::div(
                      style = "width: 50%; height: 100%; background: #f0f0f0; border-radius: 0 2px 2px 0; overflow: hidden;", # nolint
                      if (!is_negative) {
                        shiny::div(
                          style = sprintf(
                            "height: 100%%; width: %.1f%%; background: %s;",
                  ...
                  shiny::span(
                    style = sprintf("color: %s; font-size: 12px; width: 45px; text-align: right;", text_color), # nolint
                    format_number(value)
                  )
                )
              } else {
                shiny::div(
                  style = "display: flex; align-items: center; justify-content: flex-end; gap: 6px;", # nolint
                  shiny::div(
                    style = "flex: 1; max-width: 80px; height: 14px; background: #f0f0f0; border-radius: 2px; overflow: hidden;", # nolint
                    shiny::div(
                      style = sprintf(
                        "height: 100%%; width: %.1f%%; background: %s;",
                        pct, bar_color
                  ...
                  shiny::span(
                    style = sprintf("color: %s; font-size: 12px; width: 38px; text-align: right;", text_color), # nolint
                    format_number(value)
                  )
                )
              }
          ...
          reactable::reactable(
            ...
            onClick = htmlwidgets::JS(sprintf(
              "function(rowInfo, column) {
                Shiny.setInputValue('%s', {table: '%s', dim: '%s', value: rowInfo.row['%s']}, {priority: 'event'}); # nolint
              }",
        ...
        build_range_slider <- function(tbl_name, dim) {
          ...
          if (!is.null(r_range_filters()[[tbl_name]][[dim]])) {
            n_match <- sum(
              cf_vals >= r_range_filters()[[tbl_name]][[dim]][1] &
          ...
          min_label <- if (is_integer) {
            format(as.integer(full_min), big.mark = ",")
          } else {
            format_number(full_min)
          }
          max_label <- if (is_integer) {
            format(as.integer(full_max), big.mark = ",")
          } else {
            format_number(full_max)
          }

          # SVG density overlay (blue only when filters are active)
          has_any_filter <- length(r_filters()) > 0 ||
            length(r_range_filters()) > 0
          svg_w <- 300
          svg_h <- 100
          path_full <- density_to_svg_path(
            full_vals, full_min, full_max,
            svg_w, svg_h
          )
          path_cf <- if (has_any_filter) {
            density_to_svg_path(
              cf_vals, full_min, full_max,
              svg_w, svg_h
            )
          }
          ...
          shiny::div(
            ...
            shiny::tags$div(
              ...
              shiny::tags$div(
                ...
                style = "position: absolute; top: 12px; left: 0; right: 0; height: 4px; background: #e5e7eb; border-radius: 2px;" # nolint
              ),
              # Fill highlight
              shiny::tags$div(
                class = "dm-cf-dual-range-fill",
                style = "position: absolute; top: 12px; height: 4px; background: #5470c6; border-radius: 2px;" # nolint
              ),
              # Lo handle
              shiny::tags$input(
                type = "range",
                ...
                style = "position: absolute; width: 100%; top: 0; height: 28px; margin: 0; padding: 0; background: transparent; pointer-events: none; -webkit-appearance: none; appearance: none; z-index: 3;" # nolint
              ),
              # Hi handle
              shiny::tags$input(
                type = "range",
                ...
                style = "position: absolute; width: 100%; top: 0; height: 28px; margin: 0; padding: 0; background: transparent; pointer-events: none; -webkit-appearance: none; appearance: none; z-index: 4;" # nolint
              ),
              # Tooltip bubbles
            ...
            shiny::tags$div(
              style = "display: flex; justify-content: space-between; font-size: 11px; color: #999; margin-top: -2px;", # nolint
            ...
            shiny::tags$script(shiny::HTML(sprintf(
              ...
              (function() {
                ...
                function updateFill() {
                  var loVal = parseFloat(lo.value), hiVal = parseFloat(hi.value); # nolint
                ...
                function onInput() {
                  ...
                  if (isInt) { loVal = Math.round(loVal); hiVal = Math.round(hiVal); } # nolint
                  updateFill();
                  clearTimeout(debounce);
                  debounce = setTimeout(function() {
                    Shiny.setInputValue(inputId, {table: tbl, dim: dim, value: [loVal, hiVal]}, {priority: 'event'}); # nolint
                  }, 1000);
                }

                lo.addEventListener('input', function() { el.classList.add('dm-cf-active'); onInput(); }); # nolint
                hi.addEventListener('input', function() { el.classList.add('dm-cf-active'); onInput(); }); # nolint
                lo.addEventListener('change', function() { el.classList.remove('dm-cf-active'); }); # nolint
                hi.addEventListener('change', function() { el.classList.remove('dm-cf-active'); }); # nolint
                updateFill();
              })();
              ",
              slider_uid,
        ...
        build_date_slider <- function(tbl_name, dim) {
          ...
          cur_lo_num <- if (!is.null(current_range)) {
            current_range[1]
          } else {
            full_min_num
          }
          cur_hi_num <- if (!is.null(current_range)) {
            current_range[2]
          } else {
            full_max_num
          }
          ...
          svg_h <- 100
          path_full <- density_to_svg_path(
            full_vals_num, full_min_num, full_max_num,
            svg_w, svg_h
          )
          path_cf <- if (has_any_filter) {
            density_to_svg_path(
              cf_vals_num, full_min_num,
              full_max_num, svg_w, svg_h
            )
          }
          ...
          shiny::div(
            ...
            shiny::tags$div(
              ...
              shiny::tags$input(
                type = "range",
                class = "dm-cf-dual-range-lo",
                min = full_min_num, max = full_max_num,
                value = cur_lo_num, step = 1,
                style = "position: absolute; width: 100%; top: 0; height: 28px; margin: 0; padding: 0; background: transparent; pointer-events: none; -webkit-appearance: none; appearance: none; z-index: 3;" # nolint
              ),
              shiny::tags$input(
                type = "range",
        ...
        format_tbl_label <- function(tbl_name) {
          ...
          shiny::tagList(
            toupper(tbl_name),
            shiny::span(
              style = "font-weight: 400; font-size: 0.85em; color: #6b7280; margin-left: 6px; text-transform: none; letter-spacing: normal;", # nolint
              lbl
            )
          )
        }
          ...
          shiny::tags$script(shiny::HTML(sprintf(
            ...
            (function() {
              ...
              el.addEventListener('blur', function() {
                setTimeout(function() { Shiny.setInputValue('%s', false); }, 200); # nolint
              });
            })();
            ",
            ns("search_input"),
            ns("search_focus_state"),
            ns("search_focus_state")
          )))
        })
        ...
        output$search_results <- shiny::renderUI({
          ...
          for (tbl_name in info$table_names) {
            ...
            if (has_search) {
              ...
              available <- available[vapply(
                ...
                function(col) {
                  grepl(
                    search_lower, tolower(col),
                    fixed = TRUE
                  ) || (
                    !is.null(tbl_labels[[col]]) &&
                      grepl(
                        search_lower,
                        tolower(tbl_labels[[col]]),
                        fixed = TRUE
                      )
                  )
                },
          ...
          type_badge <- function(dtype) {
            ...
            info <- switch(
              ...
              "categorical" = list(
                label = "Categorical",
                class = "dm-cf-badge-categorical"
              ),
              "range" = list(
                label = "Numeric",
                class = "dm-cf-badge-numeric"
              ),
          ...
          items <- lapply(names(groups), function(tbl_name) {
            ...
            col_items <- lapply(cols, function(col_name) {
              ...
              shiny::div(
                ...
                onclick = sprintf(
                  "Shiny.setInputValue('%s', {table:'%s', dim:'%s'}, {priority:'event'}); var el=document.getElementById('%s'); el.value=''; $(el).trigger('input'); el.blur(); Shiny.setInputValue('%s', false);", # nolint
                  add_filter_id, tbl_name, col_name,
                  search_input_id, focus_state_id
                ),
                type_icon(dtype),
                shiny::span(
                  class = "dm-cf-search-item-name",
                  format_col_label(tbl_name, col_name)
                ),
                type_badge(dtype)
              )
            })
            if (length(info$table_names) > 1) {
              shiny::tagList(
                shiny::div(
                  class = "dm-cf-search-group-header",
        ...
        output$tables_grid <- shiny::renderUI({
          ...
          wrap_with_remove <- function(tbl_name, dim_name, widget) {
            shiny::div(
              ...
              shiny::div(
                ...
                shiny::span(
                  class = "dm-cf-filter-card-label",
                  format_col_label(tbl_name, dim_name)
                ),
                shiny::div(
                  ...
                  shiny::tags$button(
                    ...
                    onclick = sprintf(
                      "Shiny.setInputValue('%s', {table: '%s', dim: '%s'}, {priority: 'event'});", # nolint
          ...
          table_panels <- lapply(info$table_names, function(tbl_name) {
            ...
            if (length(range_cols) > 0) {
              parts <- c(parts, list(
                shiny::div(
                  style = "display: flex; flex-wrap: wrap; gap: 16px; margin-bottom: 12px;", # nolint
                  lapply(range_cols, function(dim) {
                    widget_id <- paste0("cf_", tbl_name, "__", dim)
                    wrap_with_remove(
                      tbl_name, dim,
                      shiny::uiOutput(ns(widget_id))
                    )
                  })
                )
              ))
          ...
          if (length(table_panels) == 0) {
            return(shiny::div(
              style = "padding: 20px; text-align: center; color: var(--blockr-color-text-muted, #6b7280); font-style: italic;", # nolint
        ...
        # --- Per-widget dynamic outputs ---
        # Only widget content re-renders on filter change.
        shiny::observe({
          active <- r_active_dims()
          col_info <- column_info_per_table()
        ...
        output$filter_status_text <- shiny::renderUI({
          ...
          shiny::tagList(
            ...
            if (has_active_dims || has_filters) shiny::span(class = "dm-cf-separator"), # nolint
              ...
              if (!is.null(tbl_cat)) {
                for (dim in names(tbl_cat)) {
                  ...
                  if (length(val) == 1) {
                    conditions <- c(
                      conditions,
                      list(call("==", as.name(dim), val))
                    )
                  } else {
                    conditions <- c(
                      conditions,
                      list(call("%in%", as.name(dim), val))
                    )
                  }
                }
              }
                  ...
                  if (!is.null(rng) && length(rng) == 2) {
                    ...
                    if (is_date_col) {
                      lo_val <- call(
                        "as.Date",
                        as.character(
                          as.Date(rng[1], origin = "1970-01-01")
                        )
                      )
                      hi_val <- call(
                        "as.Date",
                        as.character(
                          as.Date(rng[2], origin = "1970-01-01")
                        )
                      )
                    } else {
                      lo_val <- rng[1]
...
dm_crossfilter_ui <- function(id) {
  ...
  search_icon <- shiny::HTML(
    '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="7" cy="7" r="4.5"/><line x1="10.2" y1="10.2" x2="14" y2="14"/></svg>' # nolint
  )
      ...
      shiny::div(
        ...
        onclick = sprintf(
          "document.getElementById('%s').classList.toggle('expanded');
           document.querySelector('#%s .dm-cf-chevron').classList.toggle('rotated');", # nolint
          ns("advanced-options"),
          ns("advanced-toggle")
        ),
...
#'
#' @param active_dims Named list of per-table active
#'   filter columns. Each element
...
#'   When set, categorical filter bars show aggregated
...
new_dm_crossfilter_block <- function(
  ...
  blockr.core::new_transform_block(
    ...
    # Note: dat_valid intentionally omitted to avoid
    # double evaluation on startup.
...
#' @export
block_render_trigger.dm_crossfilter_block <- function(
